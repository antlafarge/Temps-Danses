...
<?php
		$dossier = stripslashes('./photos/'.$_GET['dossier']);
		...
		if (is_dir($dossier))
		{
			...
			// PHP 5.1.4 (Free)
				...
				if (preg_match('#jpg|jpeg|png|gif|bmp#i', $extension))
				{
					...
					// Affichage
					echo '<td class="miniatureGalerie"><a href="index.php?page=photo&img='.utf8_encode($fichier_url_encoded).'"><img src="'.utf8_encode($fichier_url_encoded_min).'" /><br />'.utf8_encode($liste_fichiers[$nb]).'</a></td>';
				}
